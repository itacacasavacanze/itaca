...
function findMainRequest2(resources, corsPreFlightRequestEndTime, spanEndTimeHR) {
  ...
  for (let i = 1; i < length; i++) {
    const resource = resources[i];
    const resourceStartTime = hrTimeToNanoseconds(timeInputToHrTime(resource[PerformanceTimingNames2.FETCH_START]));
    const resourceEndTime = hrTimeToNanoseconds(timeInputToHrTime(resource[PerformanceTimingNames2.RESPONSE_END]));
    const currentGap = spanEndTime - resourceEndTime;
    if (resourceStartTime >= minTime && (!bestGap || currentGap < bestGap)) {
      bestGap = currentGap;
      mainRequest = resource;
...
// node_modules/@opentelemetry/sdk-trace-web/node_modules/@opentelemetry/core/build/esm/semconv.js
...
var W3CTraceContextPropagator2 = class {
  inject(context2, carrier, setter) {
    ...
    if (!spanContext || isTracingSuppressed2(context2) || !isSpanContextValid(spanContext))
      return;
    const traceParent = `${VERSION8}-${spanContext.traceId}-${spanContext.spanId}-0${Number(spanContext.traceFlags || TraceFlags.NONE).toString(16)}`;
...
// node_modules/@opentelemetry/sdk-trace-web/node_modules/@opentelemetry/core/build/esm/trace/rpc-metadata.js
init_esm();
...
// node_modules/@opentelemetry/sdk-trace-web/node_modules/@opentelemetry/core/build/esm/utils/configuration.js
init_esm();
var logLevelMap2 = {
  ALL: DiagLogLevel.ALL,
  VERBOSE: DiagLogLevel.VERBOSE,
...
// node_modules/@opentelemetry/sdk-trace-web/node_modules/@opentelemetry/core/build/esm/internal/exporter.js
init_esm();

// node_modules/@opentelemetry/sdk-trace-web/node_modules/@opentelemetry/resources/build/esm/detect-resources.js
init_esm();

// node_modules/@opentelemetry/sdk-trace-web/node_modules/@opentelemetry/resources/build/esm/ResourceImpl.js
init_esm();
init_esm2();
...
// node_modules/@opentelemetry/sdk-trace-web/node_modules/@opentelemetry/resources/build/esm/utils.js
...
// node_modules/@opentelemetry/sdk-trace-web/node_modules/@opentelemetry/resources/build/esm/detectors/EnvDetector.js
init_esm();
init_esm2();
var EnvDetector2 = class {
  ...
  detect(_config) {
    const attributes = {};
    const rawAttributes = getStringFromEnv2("OTEL_RESOURCE_ATTRIBUTES");
    const serviceName = getStringFromEnv2("OTEL_SERVICE_NAME");
    if (rawAttributes) {
      try {
        const parsedAttributes = this._parseResourceAttributes(rawAttributes);
        Object.assign(attributes, parsedAttributes);
...
// node_modules/@opentelemetry/sdk-trace-web/node_modules/@opentelemetry/resources/build/esm/detectors/NoopDetector.js
...
var SpanImpl2 = class {
  ...
  constructor(opts) {
    ...
    this._performanceOffset = now - (this._performanceStartTime + getTimeOrigin2());
  ...
  addEvent(name, attributesOrStartTime, timeStamp) {
    ...
    const attributes = sanitizeAttributes2(attributesOrStartTime);
    this.events.push({
      name,
      attributes,
...
var ParentBasedSampler2 = class {
  ...
  constructor(config) {
    ...
    if (!this._root) {
      globalErrorHandler2(new Error("ParentBasedSampler must have a root sampler configured"));
...
var BatchSpanProcessorBase2 = class {
  ...
  constructor(_exporter, config) {
    ...
    this._maxExportBatchSize = typeof config?.maxExportBatchSize === "number" ? config.maxExportBatchSize : getNumberFromEnv2("OTEL_BSP_MAX_EXPORT_BATCH_SIZE") ?? 512;
    this._maxQueueSize = typeof config?.maxQueueSize === "number" ? config.maxQueueSize : getNumberFromEnv2("OTEL_BSP_MAX_QUEUE_SIZE") ?? 2048;
    this._scheduledDelayMillis = typeof config?.scheduledDelayMillis === "number" ? config.scheduledDelayMillis : getNumberFromEnv2("OTEL_BSP_SCHEDULE_DELAY") ?? 5e3;
    this._exportTimeoutMillis = typeof config?.exportTimeoutMillis === "number" ? config.exportTimeoutMillis : getNumberFromEnv2("OTEL_BSP_EXPORT_TIMEOUT") ?? 3e4;
...
var Tracer2 = class {
  ...
  startSpan(name, options = {}, context2 = context.active()) {
    ...
    if (isTracingSuppressed2(context2)) {
    ...
    const attributes = sanitizeAttributes2(options.attributes);
    ...
    const initAttributes = sanitizeAttributes2(Object.assign(attributes, samplingResult.attributes));
  ...
  forceFlush() {
    ...
    return new Promise((resolve) => {
      Promise.all(promises).then(() => {
        ...
        globalErrorHandler2(error || new Error("MultiSpanProcessor: forceFlush failed"));
        resolve();
      });
    });
  }
...
var BasicTracerProvider2 = class {
  ...
  constructor(config = {}) {
    const mergedConfig = merge2({}, loadDefaultConfig2(), reconfigureLimits2(config));
...
function setupPropagator(propagator) {
  ...
  if (propagator === void 0) {
    propagation.setGlobalPropagator(new CompositePropagator2({
      propagators: [
